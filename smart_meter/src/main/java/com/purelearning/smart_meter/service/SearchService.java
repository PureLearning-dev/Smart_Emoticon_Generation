...
public interface SearchService {
    ...
    /**
     * 图相似度搜索（公共广场，仅用户生成图）。图片 URL 方式。
     * 远程图片仅用于临时向量化检索，不会自动入库。
     *
     * @param url   远程图片 URL，仅支持 http/https
     * @param topK  返回数量
     * @return 按相似度排序的公共广场结果列表
     */
    List<PlazaSearchResultItem> searchByImageUrl(String url, int topK);
}
