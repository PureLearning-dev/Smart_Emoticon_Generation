...
│   │   └── MemeAssetController.java            # meme_assets 基础 CRUD（开发阶段便于验证）
│   │   │
│   │   ├── (待建) SearchController.java        # 搜索检索数据流
│   │   │                                       # - 接收搜索词 → 调 Python 向量化 → Milvus ANN → MySQL 查元数据 → 返回
│   │   │
│   │   ├── (待建) ImageSearchController.java   # 图搜图数据流
│   │   │                                       # - 接收图片 → 调 Python CLIP 向量化 → Milvus 搜索 → MySQL 查 URL/标签 → 返回
│   │   │
...
│   │   │
│   │   ├── (待建) SearchService.java           # 搜索检索：调 Python 向量化 + Milvus + MySQL
│   │   ├── (待建) ImageSearchService.java      # 图搜图：调 Python 向量化 + Milvus + MySQL
