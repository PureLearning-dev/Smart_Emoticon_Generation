...
│   │                                       # 经验：OCR 前对图片等比缩小至最长边 1024px（可配置 OCR_MAX_DIMENSION），降低上传体积与本地识别耗时
│   │
│   └── image_pipeline.py                 # 单张图片完整处理流程，串行执行
│
├── ocr_api/                               # 第三方 OCR API 调用封装
│   │                                       # 职责：仅存放外部 OCR API 客户端，不与本地模型逻辑混放
│   │
│   └── baidu_general_basic.py             # 百度通用文字识别 general_basic 调用
│
│
├── ocr/                                   # OCR 技术层
│   │                                       # 职责：图像识别流程
│   │
│   ├── engine.py                          # OCR 统一入口，按 OCR_ENGINE 分发
│   ├── paddle_ocr_legacy.py               # PaddleOCR 历史实现，OCR_ENGINE=paddle 时使用
│   ├── preprocess.py                      # 图像预处理
...
- **统一入口仅支持本地图片**：Path、str（本地路径）、bytes；不支持裸 URL，需先下载到本地或 bytes 后再传入。
- **默认百度 OCR**：`OCR_ENGINE=baidu` 时调用 `ocr_api/baidu_general_basic.py`，使用百度「通用文字识别（标准版）」`general_basic`；密钥从 `BAIDU_OCR_API_KEY` / `BAIDU_OCR_SECRET_KEY` 读取，本地 `.env` 可配置，禁止写入源码。
- **Paddle 历史实现**：`OCR_ENGINE=paddle` 时调用 `ocr/paddle_ocr_legacy.py`。PaddleOCR 模型仅在该模式下由 `init_ocr()` 预加载，避免百度模式加载本地模型。
- **管线**：下载 → 本地文件 → 缩小（`OCR_MAX_DIMENSION`）→（可选）`recognize_text_with_deadline(bytes)`：`OCR_TIMEOUT_SECONDS` 内完成识别；**超时或引擎异常**时 `ocr_text` 取 **`OCR_DEFAULT_TEXT`**；识别成功但图中无字时仍为**空串**。`OCR_ENABLED=false` 时跳过 OCR，`ocr_text` 为空串。
